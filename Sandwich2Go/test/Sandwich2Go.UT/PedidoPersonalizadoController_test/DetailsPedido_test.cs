    ...
    public class DetailsPedido_test
    {
        ...
        public void Details_Purchase_notfound(int? id)
        {
            using (context)
            {
                ...
                //Act
                var result = controller.DetailsCreado(id);
        ...
        public async Task Details_Pedido_found()
        {
            ...
            {
                //Arrange
                var expectedPurchase = new PedidoDetailsSandwichPersonalizadoViewModel(UtilitiesForPedidoDetails.GetPedidos(0, 1).First());

                var controller = new PedidosController(context);
                controller.ControllerContext.HttpContext = pedidoContext;

                //Act
                var result = await controller.DetailsCreado(UtilitiesForPedidoDetails.GetPedidos(0, 1).First().Id);

                //Assert

                var viewResult = Assert.IsType<ViewResult>(result);

                var model = viewResult.Model as PedidoDetailsSandwichPersonalizadoViewModel;
